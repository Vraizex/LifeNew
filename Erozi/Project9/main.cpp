...
void main()
{
	try {
		...
		Mat mat_SUMa = dilateAndErozia(img, 10, 8) + dilateXY(img);
		Window win_BinSuma("10 Dilate and 8 Erozia");
		win_BinSuma.show(mat_SUMa);
		
		Mat mat_erozanddite = EroziaAndDilate(img, 1, 15)- dilateEroziaLevel(img,1) - calc3x3GradientSOBOL(img) ;
		Window win_erozdit("FIRST Erozia and Ditale");
		win_erozdit.show(mat_erozanddite);

		//Mat mat_New = (img);
		//Window win_So("New Sobol");
		//win_So.show(mat_New);
