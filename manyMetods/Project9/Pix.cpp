...
cv::Mat calcLoG(cv::Mat& img)
{
	...
	for (int x = 1; x < img.cols - 1; x++)
	{
		for (int y = 1; y < img.rows - 1; y++)
		{
			...
			sub_mat.at<uint8_t>(curr_point) = (img.at<uint8_t>(Point(x + 1, y))+ img.at<uint8_t>(Point(x - 1, y)) + img.at<uint8_t>(Point(x , y + 1)) + img.at<uint8_t>(Point(x, y - 1)) - 4 * img.at<uint8_t>(curr_point))/2 - 250;
