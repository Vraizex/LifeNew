	...
	for (int x = 1; x < img.cols - 1; x++)
	{
		for (int y = 1; y < img.rows - 1; y++)
		{
			for (int N = -1; N <= 1; N++)
			{
				...
				//mat_mod.at<uint8_t>(curr_point) = (pow(pow(img.at<uint8_t>(curr_point), 2) - pow(img.at<uint8_t>(Point(x + N, y + N)), 2), 1 / 2) + 255) / 2;
				
				mat_mod.at<uint8_t>(curr_point) = (pow(pow(neighPixel, 2) - pow(startPixel, 2), 1 / 2) + 255) / 2;
						
				double angle = atan2(img.at<uint8_t>(Point(x + N)) - img.at<uint8_t>(Point(x)), img.at<uint8_t>(Point(y + N)) - img.at<uint8_t>(Point(y)));
				double angles = angle * 180 / M_PI;
